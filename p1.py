...
def handle_player_move(player_pos, rolled_dice, player_queue, player_x, player_y):
    ...
    # If the new position exceeds 100, stay in the current position
    if new_pos > 100:
    ...
    for ladder in ladders:
        if new_pos == ladder["start"]:
            # Move from current position to the base of the ladder box by box
            while player_pos < ladder["start"]:
                player_pos += 1
                update_queue(player_pos - 1, player_pos, player_queue)

            # Now move from the base of the ladder to the top
    ...
    for snake in snakes:
        if new_pos == snake["start"]:
            # Move from current position to the head of the snake box by box
            while player_pos < snake["start"]:
                player_pos += 1
                update_queue(player_pos - 1, player_pos, player_queue)

            # Now move from the head of the snake to the tail
            player_pos = snake["end"]
            update_queue(snake["start"], player_pos, player_queue, snake=snake)
            return player_pos, player_queue, player_x, player_y

    # Normal movement when no ladder or snake is encountered
