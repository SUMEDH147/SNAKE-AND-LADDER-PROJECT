...
player1_jump_speed=80
player2_jump_speed=80
...
while running:
    ...
    if not game_started:
        ...
        def animate_movement(player_x, player_y, target_x, target_y, game_element):
    # Move player to the start of the game element (ladder or snake)
            while (player_x, player_y) != (target_x, target_y):
        # Update player position
                dx = target_x - player_x
                dy = target_y - player_y
                dist = math.sqrt(dx**2 + dy**2)
                player_x += dx / dist * player_jump_speed
                player_y += dy / dist * player_jump_speed

    # Animate climbing the ladder or sliding down the snake
            if game_element == "ladder":
        # Get the end position of the ladder
                end_x = ((ladder_end_pos - 1) % 10) * box_size + 30
                end_y = screen_height - (((ladder_end_pos - 1) // 10) % 10 + 1) * box_size + 30
            elif game_element == "snake":
        # Get the end position of the snake
                end_x = ((snake_end_pos - 1) % 10) * box_size + 30
                end_y = screen_height - (((snake_end_pos - 1) // 10) % 10 + 1) * box_size + 30

            while (player_x, player_y) != (end_x, end_y):
        # Update player position
                dx = end_x - player_x
                dy = end_y - player_y
                dist = math.sqrt(dx**2 + dy**2)
        ...
        if player1_pos in ladder:
            ladder_end_pos = ladders[player1_pos]
            player1_x, player1_y = animate_movement(player1_x, player1_y, ((player1_pos - 1) % 10) * box_size + 30, screen_height - (((player1_pos - 1) // 10) % 10 + 1) * box_size + 30, "ladder")
            player1_pos = ladder_end_pos

        elif player1_pos in snake:
            snake_end_pos = snakes[player1_pos]
            player1_x, player1_y = animate_movement(player1_x, player1_y, ((player1_pos - 1) % 10) * box_size + 30, screen_height - (((player1_pos - 1) // 10) % 10 + 1) * box_size + 30, "snake")
            player1_pos = snake_end_pos
