...
def calculate_position(position):
    x = (position - 1) % 10 * 80 + 25
    y = (position - 1) // 10 * 80 + 25
    return x, y
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if play_button_rect.collidepoint(event.pos):
                ...
                rolling = True
                dice_animation_time = pygame.time.get_ticks()  # Start animation timer
                rolled_dice = None  # Will be set after rolling animation

    # Update game state
    if rolling:
        elapsed_time = pygame.time.get_ticks() - dice_animation_time
        if elapsed_time < 1000:  # 1.5 seconds for animation
            # Rolling animation
            screen.fill(white)
            num_frames = 30
    ...
    if not game_started:
        ...
        # Draw dice image as button
        
        if rolling:
            elapsed_time=pygame.time.get_ticks()-dice_animation_time
            nul_frames=30
            frame_index=(elapsed_time//(1500//num_frames))%6
            screen.blit(dice_images[frame_index],(dice_button_x,dice_button_y))
            roll_sound.play()
        else:
