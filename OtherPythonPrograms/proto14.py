    ...
    if not game_started:
        ...
        for ladder in ladders:
            ...
            for i in range(1,6):
                ...
                y=start_y+(i*spacing)
                pygame.draw.line(screen, (0, 255, 255), (x,y), (x + 40, y), 3)               
            pygame.draw.line(screen, (0, 255, 255), (start_x1, start_y), (end_x1, end_y), 7)
            pygame.draw.line(screen, (0, 255, 255), (start_x2, start_y), (end_x2, end_y), 7)
    if game_started:
        if player1_target_x is not None:
            dx = player1_target_x - player1_x
            dy = player1_target_y - player1_y
            dist = math.sqrt(dx**2 + dy**2)
            if dist > player1_move_speed:
                player1_x += dx / dist * player1_move_speed
                player1_y += dy / dist * player1_move_speed
            else:
            ...
            player1_x = round(((player1_pos - 1) % 10) * box_size + 30)
            player1_y = round(screen_height - (((player1_pos - 1) // 10) % 10 + 1) * box_size + 10)
            if (player1_pos - 1) // 10 % 2 == 1:
                player1_x = round((9 - ((player1_pos - 1) % 10)) * box_size + 10)
                
        if player2_target_x is not None:
            dx = player2_target_x - player2_x
            dy = player2_target_y - player2_y
            dist = math.sqrt(dx**2 + dy**2)
            if dist > player2_move_speed:
                player2_x += dx / dist * player2_move_speed
                player2_y += dy / dist * player2_move_speed
            else:
            ...
            player2_x = round(((player2_pos - 1) % 10) * box_size + 30)
            player2_y = round(screen_height - (((player2_pos - 1) // 10) % 10 + 1) * box_size + 10)
            if (player2_pos - 1) // 10 % 2 == 1:
                player2_x = round((9 - ((player2_pos - 1) % 10)) * box_size + 10)

        
        screen.blit(player1_symbol, (int(player1_x), int(player1_y)))
        screen.blit(player2_symbol, (int(player2_x), int(player2_y)))
