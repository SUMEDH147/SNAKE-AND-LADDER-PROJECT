...
def draw_dice(screen, image, angle):
    ...
    rect = rotated_image.get_rect(center=(dice_button_x + dice_button_width // 2, dice_button_y + dice_button_height // 2))
                ...
                if player1_pos + rolled_dice > 100:
                    ...
                    for ladder in ladders:
                        if new_pos == ladder["start"]:
                            ladder_start_x, ladder_start_y = calculate_position(ladder["start"])
                            ladder_end_x, ladder_end_y = calculate_position(ladder["end"])
                            while (player1_x, player1_y) != (ladder_start_x, ladder_start_y):
                                if player1_x < ladder_start_x:
                                    player1_x += box_size
                                elif player1_x > ladder_start_x:
                                    player1_x -= box_size
                                if player1_y < ladder_start_y:
                                    player1_y += box_size
                                elif player1_y > ladder_start_y:
                                    player1_y -= box_size
                                screen.fill(white)
                                game_board()
                            ...
                            for pos in range(ladder["start"], ladder["end"]):
                                player1_x, player1_y = calculate_position(pos)
                                screen.fill(white)
                                game_board()
                                draw_snakes(screen, snakes)
                                draw_ladders(screen, ladders)
                ...
                if player2_pos + rolled_dice > 100:
                    ...
                    for ladder in ladders:
                        if new_pos == ladder["start"]:
                            ladder_start_x, ladder_start_y = calculate_position(ladder["start"])
                            ladder_end_x, ladder_end_y = calculate_position(ladder["end"])
                            while (player2_x, player2_y) != (ladder_start_x, ladder_start_y):
                                if player2_x < ladder_start_x:
                                    player2_x += box_size
                                elif player2_x > ladder_start_x:
                                    player2_x -= box_size
                                if player2_y < ladder_start_y:
                                    player2_y += box_size
                                elif player2_y > ladder_start_y:
                                    player2_y -= box_size
                                screen.fill(white)
                                game_board()
                                draw_snakes(screen, snakes)
                                ...
                                pygame.time.delay(200)
                            for pos in range(ladder["start"], ladder["end"]):
                                player2_x, player2_y = calculate_position(pos)
                                screen.fill(white)
                                game_board()
                                draw_snakes(screen, snakes)
                                draw_ladders(screen, ladders)
